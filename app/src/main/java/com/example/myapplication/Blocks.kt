...
import com.example.myapplication.MainActivity.Companion.functionReturnValues
...
sealed class DataBlocks : MainActivity() {
    ...
    data class If(
        var el1: String = "", var el2: String = "", var choose: String = "",
        var listIf: MutableList<DataBlocks> = mutableListOf()
    ) : DataBlocks() {
        fun addElse() {
    ...
    data class Cycle(
        var el1: String = "", var el2: String = "", var choose: String = "",
        var listCycle: MutableList<DataBlocks> = mutableListOf()
    ) : DataBlocks()

    data class Function(
        var name: String = "", var elem: String = "",
        var listFun: MutableList<DataBlocks> = mutableListOf()
    ) : DataBlocks() {
        fun addReturn() {
    ...
    fun doFunctionBlock(
        ...
        text: TextView,
        index: Int,
        blocks: MutableList<DataBlocks>,
        nameOfFunction: String
    ) {
        var str: String = text.text.toString()
        when (block) {
            is DataBlocks.Return -> {
                functionReturnValues[nameOfFunction] =
                    Expression(block.return_name, text).valueOfExpression.toInt()
                return
            }
            else -> doProgram(block, text, index, blocks)
        }
    }
    ...
    fun doProgram(
        ...
        when (block) {
            ...
            is DataBlocks.InitArray -> {
                val a = StaticArray(block.name, block.len, text)
            ...
            is DataBlocks.OutputEl -> {
                val a = OutputBlock(block.name, text)
            ...
            is DataBlocks.AssigmentEl -> {
                val a = AssignmentOperation(block.el1, block.el2, text)
            ...
            is DataBlocks.InputEl -> {
                val a = InputBlock(block.name, text)
            ...
            is DataBlocks.Cycle -> {
                val a =
                    Cycle(block.el1, block.choose, block.el2, getCommands(blocks, index + 1), text)
            ...
            is DataBlocks.Function -> {
                val a = FunctionDeclarator(block.name, block.elem, getCommands(blocks, index + 1))
